...
- Use `plugin-kit-ai init --platform gemini --runtime go` only when you intentionally want the `public-beta` Go hook lane for `SessionStart`, `SessionEnd`, `BeforeAgent`, `AfterAgent`, `BeforeTool`, and `AfterTool`
