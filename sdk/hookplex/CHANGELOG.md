...
- Root package `hookplex` now acts as composition/runtime only.
- Public registration moved from root-Claude methods to platform registrars:
  ...
  - `app.Codex().OnNotify(...)`
- App construction now uses `hookplex.New(hookplex.Config{...})`.
...
```go
app := hookplex.New(hookplex.Config{Name: "my-plugin"})
app.Claude().OnStop(...)
```
