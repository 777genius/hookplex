...
import (
	...
	"github.com/hookplex/hookplex/sdk/internal/descriptors/defs"
...
func RenderArtifacts() ([]Artifact, error) {
	...
	artifacts = append(artifacts,
		Artifact{Path: "sdk/hookplex/internal/descriptors/gen/registry_gen.go", Content: mustGo(renderRegistry(m))},
		Artifact{Path: "sdk/hookplex/internal/descriptors/gen/resolvers_gen.go", Content: mustGo(renderResolvers(m))},
		Artifact{Path: "sdk/hookplex/internal/descriptors/gen/support_gen.go", Content: mustGo(renderSupport(m))},
		Artifact{Path: "sdk/hookplex/internal/descriptors/gen/completeness_gen_test.go", Content: mustGo(renderCompletenessTest(m))},
		Artifact{Path: "cli/hookplex/internal/scaffold/platforms_gen.go", Content: mustGo(renderScaffoldPlatforms(m))},
		Artifact{Path: "cli/hookplex/internal/validate/rules_gen.go", Content: mustGo(renderValidateRules(m))},
		Artifact{Path: "docs/generated/support_matrix.md", Content: []byte(renderSupportMatrix(m))},
	)
	for _, p := range runtimeProfiles(m) {
		artifacts = append(artifacts, Artifact{
			Path:    fmt.Sprintf("sdk/hookplex/%s/registrar_gen.go", p.PublicPackage),
...
func renderRegistry(m model) string {
	...
	b.WriteString("\t\"github.com/hookplex/hookplex/sdk/internal/runtime\"\n")
	b.WriteString(")\n\n")
...
func renderSupport(m model) string {
	var b strings.Builder
	b.WriteString("package gen\n\n")
	b.WriteString("import \"github.com/hookplex/hookplex/sdk/internal/runtime\"\n\n")
...
func renderCompletenessTest(m model) string {
	...
	b.WriteString("\t\"github.com/hookplex/hookplex/sdk/internal/descriptors/defs\"\n")
	b.WriteString(")\n\n")
...
func FindRepoRoot(start string) (string, error) {
	...
	for {
		...
		if err == nil && bytes.HasPrefix(b, []byte("module github.com/hookplex/hookplex\n")) {
			return dir, nil
		}
