...
// HOOKPLEX_E2E_TRACE, when set to a file path, appends one JSON line per hook invocation (for CLI e2e).

func trace(rec map[string]any) {
	p := os.Getenv("HOOKPLEX_E2E_TRACE")
	if p == "" {
		return
	}
...
func main() {
	app := hookplex.New(hookplex.Config{Name: "hookplex-e2e"})
	...
	app.Claude().OnPreToolUse(func(e *claude.PreToolUseEvent) *claude.PreToolResponse {
		...
		if sub := strings.TrimSpace(os.Getenv("HOOKPLEX_E2E_PRETOOL_DENY_SUBSTRING")); sub != "" && strings.Contains(ti.Command, sub) {
			trace(map[string]any{"hook": "PreToolUse", "outcome": "deny", "command": ti.Command, "match": sub})
			return claude.PreToolDeny("blocked: hookplex CLI integration marker")
		}
