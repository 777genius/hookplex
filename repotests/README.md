...
- **Gemini / extension lifecycle:** opt-in real CLI smoke — **`PLUGIN_KIT_AI_RUN_GEMINI_CLI=1`** плюс **`PLUGIN_KIT_AI_E2E_GEMINI=/path/to/gemini`**. Для backward compatibility также подходят **`PLUGIN_KIT_AI_GEMINI_BIN`** и **`GEMINI_BIN`**. При необходимости можно явно выключить через **`PLUGIN_KIT_AI_SKIP_GEMINI_CLI=1`**. Тест работает в `Temp HOME` и проверяет `gemini extensions link|config|disable|enable` против rendered reference extension, не трогая реальный user state.
- **Gemini / runtime hooks:** extra opt-in real CLI smoke — **`PLUGIN_KIT_AI_RUN_GEMINI_RUNTIME_LIVE=1`** плюс `PLUGIN_KIT_AI_E2E_GEMINI` или legacy `PLUGIN_KIT_AI_GEMINI_BIN`/`GEMINI_BIN`. Тест собирает generated Gemini Go runtime repo, линкует extension в isolated home и проверяет реальный `SessionStart`, model-path (`BeforeModel`/`AfterModel`), tool-selection path (`BeforeToolSelection`), agent-path (`BeforeAgent`/`AfterAgent`) и tool-path (`BeforeTool`/`AfterTool`) dispatch через trace helper. Advisory `Notification` и `PreCompress` сейчас закреплены deterministic local smoke/canary path, а не live dispatch.
