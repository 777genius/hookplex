...
- **Gemini / runtime hooks:** extra opt-in real CLI smoke — **`PLUGIN_KIT_AI_RUN_GEMINI_RUNTIME_LIVE=1`** плюс `PLUGIN_KIT_AI_E2E_GEMINI` или legacy `PLUGIN_KIT_AI_GEMINI_BIN`/`GEMINI_BIN`. Тест собирает generated Gemini Go runtime repo, линкует extension в isolated home и проверяет реальный `SessionStart` hook dispatch через trace helper.
