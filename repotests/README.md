...
- Those same checked-in production examples now also bridge into the mutable CLI config path: `TestCodexProductionExampleRuntimeMCPAddGetListRemoveInIsolatedHome` and `TestCodexPackageProductionExampleMCPAddGetListRemoveInIsolatedHome` take MCP truth from rendered reference examples and verify real `codex mcp add|get|list|remove` in isolated config state.
