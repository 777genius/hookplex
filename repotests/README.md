...
- **Gemini / runtime hooks:** extra opt-in real CLI smoke — **`PLUGIN_KIT_AI_RUN_GEMINI_RUNTIME_LIVE=1`** плюс `PLUGIN_KIT_AI_E2E_GEMINI`. Тест собирает generated Gemini Go runtime repo, линкует extension в isolated home и проверяет production-ready stable subset: `SessionStart`, `SessionEnd`, model-path (`BeforeModel`/`AfterModel`), tool-selection path (`BeforeToolSelection`), agent-path (`BeforeAgent`/`AfterAgent`) и tool-path (`BeforeTool`/`AfterTool`) через trace helper, включая documented `tool_input`/`tool_response` payload presence. Deterministic repo-local Gemini smoke дополнительно фиксирует `original_request_name`, `mcp_context`, runtime control semantics, и runtime transform semantics. Advisory `Notification` и `PreCompress` остаются `public-beta` и сейчас закреплены deterministic local smoke/canary path, а не live dispatch.
