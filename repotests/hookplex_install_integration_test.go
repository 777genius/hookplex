...
func TestHookplexInstall_MockGitHub(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
...
func TestHookplexInstall_defaultDir(t *testing.T) {
	requireBindTests(t)
	archName := fmt.Sprintf("plug_1.0.0_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	binName := "plug"
	tarGz := mustTarGz(t, binName, []byte("plugbin"))
	...
	code, out := runInstall(t, hookplexBin, workDir, srv.URL, "--tag", "v1", "--force")
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestHookplexInstall_outputName(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
	archName := fmt.Sprintf("plug_1.0.0_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	binName := "plug"
	...
	hookplexBin := buildHookplex(t)
	outDir := t.TempDir()
	code, out := runInstall(t, hookplexBin, "", srv.URL, "--tag", "v1", "--dir", outDir, "--force", "--output-name", "renamed-bin")
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestHookplexInstall_redirectAsset(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
	archName := fmt.Sprintf("plug_1.0.0_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	binName := "plug"
...
func TestHookplexInstall_checksum429ThenOK(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
	archName := fmt.Sprintf("plug_1.0.0_%s_%s.tar.gz", runtime.GOOS, runtime.GOARCH)
	binName := "plug"
...
func TestHookplexInstall_latestRawBinary(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
	...
	hookplexBin := buildHookplex(t)
	outDir := t.TempDir()
	code, out := runInstall(t, hookplexBin, "", srv.URL, "--latest", "--dir", outDir, "--force")
	if code != 0 {
		t.Fatalf("exit %d: %s", code, out)
	}
...
func TestHookplexInstall_API404(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
...
func TestHookplexInstall_API403(t *testing.T) {
	t.Parallel()
	requireBindTests(t)
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
