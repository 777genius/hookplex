...
func TestGeminiE2ETraceCapturesRuntimeControlSemantics(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			hook:        "Notification",
			payload:     `{"session_id":"s","cwd":".","hook_event_name":"Notification","notification_type":"ToolPermission","message":"approve?","details":{"tool_name":"read_file"}}`,
			envKey:      "PLUGIN_KIT_AI_E2E_GEMINI_NOTIFICATION",
			envValue:    "message:notify from e2e",
			wantOutcome: "message",
			wantSubstrs: []string{`"systemMessage":"notify from e2e"`},
		},
		{
			name:        "GeminiPreCompress",
			hook:        "PreCompress",
			payload:     `{"session_id":"s","cwd":".","hook_event_name":"PreCompress","trigger":"manual"}`,
			envKey:      "PLUGIN_KIT_AI_E2E_GEMINI_PRE_COMPRESS",
			envValue:    "message:compress from e2e",
			wantOutcome: "message",
			wantSubstrs: []string{`"systemMessage":"compress from e2e"`},
		},
		{
			name:        "GeminiSessionStart",
			hook:        "SessionStart",
