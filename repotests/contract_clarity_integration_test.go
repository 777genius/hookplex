...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, string(cliReadme), "Builds the **`plugin-kit-ai`** binary: `init`, `bootstrap`, `doctor`, `export`, `bundle install`, `render`, `import`, `inspect`, `normalize`, `validate`, `capabilities`, `install`, `version`")
	...
	mustContain(t, string(cliReadme), "`plugin-kit-ai bundle install` is the `public-beta` local bundle installer for exported Python/Node handoff archives")
	...
	mustContain(t, string(supportDoc), "- `plugin-kit-ai bundle install` for local exported Python/Node bundles")
	...
	mustContain(t, string(supportDoc), "stable scope is scaffold, validate, launcher execution, repo-local bootstrap, read-only doctor checks, and bounded portable export bundles")
	mustContain(t, string(supportDoc), "beta local-runtime remainder:")
	mustContain(t, string(supportDoc), "local bundle-install beta surface:")
	...
	mustContain(t, string(hardeningDoc), "local exported bundle install for Python/Node remains `public-beta`")
	mustContain(t, string(releaseDoc), "stable Node/Python doctor/bootstrap/export claims, beta local bundle-install claims")
	...
	mustContain(t, string(releaseChecklist), "beta local bundle-install claims")
	...
	mustContain(t, string(abiDoc), "`plugin-kit-ai bundle install` is the `public-beta` local bundle installer for exported `python`/`node` handoff bundles")
