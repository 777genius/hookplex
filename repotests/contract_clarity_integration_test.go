...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, matrix, "| gemini | SessionEnd | runtime_supported | beta | runtime-supported but not stable | false |")
	mustContain(t, matrix, "| gemini | BeforeModel | runtime_supported | beta | runtime-supported but not stable | false |")
	mustContain(t, matrix, "| gemini | AfterModel | runtime_supported | beta | runtime-supported but not stable | false |")
	mustContain(t, matrix, "| gemini | BeforeToolSelection | runtime_supported | beta | runtime-supported but not stable | false |")
	mustContain(t, matrix, "| gemini | BeforeAgent | runtime_supported | beta | runtime-supported but not stable | false |")
	mustContain(t, matrix, "| gemini | AfterAgent | runtime_supported | beta | runtime-supported but not stable | false |")
	...
	mustContain(t, targetMatrix, "package_metadata, interface, manifest_extra, app_manifest")
	mustContain(t, targetMatrix, "package_metadata=targets/codex-package/package.yaml")
	mustContain(t, targetMatrix, "interface=targets/codex-package/interface.json")
	mustContain(t, targetMatrix, "config_extra=targets/codex-runtime/config.extra.toml")
	mustContain(t, targetMatrix, "interface=stable, manifest_extra=stable, app_manifest=beta")
	...
	mustContain(t, targetMatrix, "| opencode | code_plugin | workspace_config_lane | ignored | workspace | workspace config file | config authoring workspace | config reload or restart | opencode.json | packaging-only target | workspace-config lane with first-class npm plugin refs, MCP, skills, commands, agents, themes, beta standalone tools with dedicated live evidence, stable official-style local JS/TS plugins plus shared dependency metadata, JSON/JSONC native import, explicit user-scope and env-config import compatibility, permission-first passthrough config semantics, and beta custom tools across standalone tools and plugin code |")
	mustContain(t, targetMatrix, "mcp=stable, rules=stable, agents_md=stable, claude_md=unsupported")
	mustContain(t, targetMatrix, ".cursor/mcp.json (when portable MCP is authored), .cursor/rules/** (when rules are authored), AGENTS.md (when agents_md is authored)")
	...
	assertCapabilityContract(t, byKey, "gemini/SessionEnd", "beta", "runtime-supported but not stable")
	assertCapabilityContract(t, byKey, "gemini/BeforeModel", "beta", "runtime-supported but not stable")
	assertCapabilityContract(t, byKey, "gemini/AfterModel", "beta", "runtime-supported but not stable")
	assertCapabilityContract(t, byKey, "gemini/BeforeToolSelection", "beta", "runtime-supported but not stable")
	assertCapabilityContract(t, byKey, "gemini/BeforeAgent", "beta", "runtime-supported but not stable")
	assertCapabilityContract(t, byKey, "gemini/AfterAgent", "beta", "runtime-supported but not stable")
	...
	mustContain(t, string(sdkStabilityDoc), "approved exported Gemini helper constructors for the current 9-hook beta runtime surface")
	...
	mustContain(t, string(cliReadme), "builds the **`plugin-kit-ai`** binary: `init`, `bootstrap`, `doctor`, `dev`, `test`, `export`, `bundle install`, `bundle fetch`, `bundle publish`, `render`, `import`, `inspect`, `normalize`, `validate`, `capabilities`, `install`, `version`, plus experimental `skills` authoring commands")
	...
	mustContain(t, string(supportDoc), "Gemini runtime: optional `public-beta` Go runtime lane")
