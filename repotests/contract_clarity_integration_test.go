...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, targetMatrix, "| opencode | code_plugin | workspace_config_lane | ignored | workspace | workspace config file | config authoring workspace | config reload or restart | opencode.json | packaging-only target | workspace-config lane with first-class npm plugin refs, MCP, validated mirrored skills, JSON/JSONC native config import, and config passthrough; no first-class local plugin code or workspace directory support |")
	mustContain(t, targetMatrix, "agent_config=passthrough_only")
	mustContain(t, targetMatrix, "tools_config=passthrough_only")
	mustContain(t, targetMatrix, "commands=unsupported")
	mustContain(t, targetMatrix, "agents=unsupported")
	mustContain(t, targetMatrix, "themes=unsupported")
	...
	mustContain(t, string(releaseDoc), "stable Node/Python doctor/bootstrap/export/bundle-install claims, beta remote bundle-fetch claims")
