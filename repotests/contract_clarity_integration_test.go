...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, targetMatrix, "| opencode | code_plugin | workspace_config_lane | ignored | workspace | workspace config file | config authoring workspace | config reload or restart | opencode.json | packaging-only target | workspace-config lane with first-class npm plugin refs, MCP, skills, commands, agents, themes, official-style local JS/TS plugins, JSON/JSONC native import, explicit opt-in user-scope import, config passthrough, and beta plugin-local dependency/custom-tool support |")
	...
	mustContain(t, targetMatrix, "local_plugin_code=beta")
	mustContain(t, targetMatrix, "custom_tools=beta")
	mustContain(t, targetMatrix, "local_plugin_dependencies=beta")
	...
	mustContain(t, string(rootReadme), "curl -fsSL https://raw.githubusercontent.com/plugin-kit-ai/plugin-kit-ai/main/scripts/install.sh | sh")
	mustContain(t, string(rootReadme), "The official bootstrap path for the `plugin-kit-ai` CLI itself is `scripts/install.sh`")
	mustContain(t, string(rootReadme), "`plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1`")
	...
	mustContain(t, string(cliReadme), "`plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1` reuses that same verified release contract")
	...
	mustContain(t, string(supportDoc), "`plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1` is the official CI setup action")
	...
	mustContain(t, string(supportDoc), "CI bootstrap uses `plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1`")
	...
	mustContain(t, string(interpretedPromotionDoc), "official downstream CLI availability through `scripts/install.sh` and `plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1`")
	...
	mustContain(t, string(productionDoc), "Use `scripts/install.sh` to bootstrap the `plugin-kit-ai` CLI locally")
	mustContain(t, string(productionDoc), "`plugin-kit-ai/plugin-kit-ai/setup-plugin-kit-ai@v1`")
	...
	mustContain(t, string(releaseDoc), "official `plugin-kit-ai` bootstrap/setup path evidence (`scripts/install.sh`, `setup-plugin-kit-ai@v1`, generated bundle-release workflow)")
