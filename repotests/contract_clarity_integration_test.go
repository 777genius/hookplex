...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, targetMatrix, "| cursor | code_plugin | workspace_config_lane | ignored | workspace | workspace config files | config authoring workspace | config reload or restart | .cursor/mcp.json | packaging-only target | workspace-config lane with first-class MCP config and project rules |")
	...
	mustContain(t, targetMatrix, "package_metadata=src/targets/codex-package/package.yaml")
	mustContain(t, targetMatrix, "interface=src/targets/codex-package/interface.json")
	mustContain(t, targetMatrix, "config_extra=src/targets/codex-runtime/config.extra.toml")
	...
	mustContain(t, string(productionDoc), "Cursor: workspace-config-only target with repo-local `.cursor/mcp.json` and project-root `.cursor/rules/**`; root `AGENTS.md` is reserved for plugin boundary docs, not a Cursor-native target surface")
	...
	mustContain(t, string(productionDoc), "Cursor: treat `generate --check` plus `validate --strict` as the repo-local readiness gate for the documented workspace-config subset")
	...
	mustContain(t, string(supportDoc), "OpenCode local plugin loading stable subset is guarded by `generate --check`, strict validation, the production example canary, and the documented `test-opencode-live` smoke path")
