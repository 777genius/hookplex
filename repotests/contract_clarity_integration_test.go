...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, string(rootReadme), "https://github.com/777genius/plugin-kit-ai-starter-codex-go")
	...
	mustContain(t, string(rootReadme), "https://github.com/777genius/plugin-kit-ai-starter-claude-go")
	...
	mustContain(t, string(rootReadme), "first-class standalone tools in `public-beta`")
	mustContain(t, string(rootReadme), "layered project/user/env config-source import fidelity")
	...
	mustContain(t, string(rootReadme), "`plugin-kit-ai-runtime` on PyPI and npm")
	mustContain(t, string(rootReadme), "Delivery-mode guide: [docs/CHOOSING_HELPER_DELIVERY_MODE.md](docs/CHOOSING_HELPER_DELIVERY_MODE.md)")
	mustContain(t, string(rootReadme), "./bin/plugin-kit-ai init my-plugin --platform codex-runtime --runtime python --runtime-package")
	mustContain(t, string(rootReadme), "./bin/plugin-kit-ai init my-plugin --platform codex-runtime --runtime node --typescript --runtime-package")
	...
	mustContain(t, string(rootReadme), "pipx install plugin-kit-ai")
	...
	mustContain(t, string(rootReadme), "When the PyPI wrapper has been published for a release, the Python ecosystem path is `pipx install plugin-kit-ai` or `pipx run plugin-kit-ai version`")
	...
	mustContain(t, string(rootReadme), "| `python` | stable local-runtime subset | repo-local executable ABI on `codex-runtime` and `claude` | Python `3.10+`; lockfile-first manager detection; `venv`/`requirements`/`uv` use repo-local `.venv`, `poetry`/`pipenv` can use manager-owned envs |")
	...
	mustContain(t, string(rootReadme), "`examples/starters`: copy-first Go/Python/Node starter repos")
	...
	mustContain(t, string(cliReadme), "./bin/plugin-kit-ai init my-plugin --platform codex-runtime --runtime python --runtime-package")
	mustContain(t, string(cliReadme), "./bin/plugin-kit-ai init my-plugin --platform codex-runtime --runtime node --typescript --runtime-package")
	...
	mustContain(t, string(startersExamplesReadme), "copy-first entrance for stable Go, Python, and Node authoring on Codex and Claude")
