...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, string(supportDoc), "A stronger auth-seeded variant now also proves that copied live auth keeps `codex login status` plus `mcp add|get|list` working inside a temporary `CODEX_HOME`, while the subsequent `codex exec` probe can still skip because the persisted MCP tool is not exposed in that session")
