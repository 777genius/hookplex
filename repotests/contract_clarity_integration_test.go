...
func TestContractClarity_RuntimeMetadataAndDocsStayAligned(t *testing.T) {
	...
	mustContain(t, string(supportDoc), "real `codex exec` with explicit `-c notify=...` override still reaches the repository-owned notify hook harness, real `codex mcp get --json` with explicit `-c mcp_servers...` overrides still exposes the projected MCP server contract")
	mustContain(t, string(supportDoc), "a rendered `codex-package` `.mcp.json` sidecar can be projected into those documented overrides for real `mcp get` plus `exec` MCP smoke")
	mustContain(t, string(supportDoc), "current live Codex CLI build (`v0.117.0` in repo evidence), `codex exec` and `codex mcp get` do not reliably honor project-local `.codex/config.toml`")
	...
	mustContain(t, string(makefile), "^TestGeminiCLIRuntimeSessionStart")
