...
- `import` and `normalize` matter when bringing native config into the managed project model
- bundle commands matter for portable Python and Node handoff flows

Start with [Quickstart](/en/guide/quickstart) when you need the shortest path, or [Import Existing Native Config](/en/guide/import-existing-native-config) when you are entering from current native target files.
