...
<MermaidDiagram
  :chart="`
flowchart LR
  Init[init or import] --> Render[render]
...
- `import` и `normalize` важны, когда нужно перенести native config в управляемую модель проекта
- bundle commands важны для portable Python и Node handoff flows

Начинайте с [Быстрого старта](/ru/guide/quickstart), если нужен самый короткий путь, или с [Импорта существующего native config](/ru/guide/import-existing-native-config), если вы входите с текущими native target files.
