...
function localizeCommandShort(short, commandPath, locale) {
  ...
  const translations = new Map([
    ...
    ["Compile native target artifacts from the package graph discovered via canonical src/plugin.yaml plus the standard authored directories.", "Собирает нативные артефакты целевых платформ из package graph, используя канонический `src/plugin.yaml` и стандартные authored directories."],
    [". discovered via canonical src/plugin.yaml plus the standard authored directories.", ", используя канонический `src/plugin.yaml` и стандартные authored directories."],
