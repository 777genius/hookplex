...
func All() []Entry {
	return fromSupport(hookplex.Supported())
}
...
func fromSupport(entries []hookplex.SupportEntry) []Entry {
...
func capabilities(in []hookplex.CapabilityID) []string {
...
func transportModes(in []hookplex.TransportMode) []string {
	out := make([]string, 0, len(in))
	for _, mode := range in {
