...
func runtimeTestUnsupportedPlatformError(enabledTargets []string, requested string) error {
	if isGeminiRuntimeTarget(requested, enabledTargets) {
		return fmt.Errorf("plugin-kit-ai test currently covers only fixture-driven runtime targets: claude or codex-runtime. Gemini uses its dedicated runtime gate instead: go test ./..., plugin-kit-ai generate --check ., plugin-kit-ai validate . --platform gemini --strict, plugin-kit-ai inspect . --target gemini, plugin-kit-ai capabilities --mode runtime --platform gemini, make test-gemini-runtime, then gemini extensions link . and optionally make test-gemini-runtime-live")
	}
