...
func init() {
	...
	initCmd.Flags().BoolVar(&initClaudeExtendedHooks, "claude-extended-hooks", false, "for --platform claude, scaffold the full runtime-supported hook set instead of the stable default subset")
}
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	out, err := initRunner.Run(app.InitOptions{
		...
		Force:               initForce,
		Extras:              initExtras,
		ClaudeExtendedHooks: initClaudeExtendedHooks,
	})
	if err != nil {
		return err
	}
