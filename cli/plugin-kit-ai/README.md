...
- `render`: render native Claude artifacts, Codex package/runtime lane artifacts, Gemini CLI extension packaging artifacts, OpenCode workspace config artifacts, and Cursor workspace config artifacts from `plugin.yaml` plus `targets/<platform>/...`; when `publish/codex/marketplace.yaml` is authored for `codex-package`, `render` also produces repo-level `.agents/plugins/marketplace.json`, and when `publish/claude/marketplace.yaml` is authored for `claude`, `render` also produces `.claude-plugin/marketplace.json`
...
- `publish`: first-class bounded publish workflow for documented publication channels; `publish --channel codex-marketplace|claude-marketplace --dest <marketplace-root>` materializes the corresponding local marketplace root, `--dry-run` previews the write, `publish --channel gemini-gallery --dry-run` emits a repository/release publication plan instead of pretending Gemini has a local marketplace-root flow, and `publish --format json` emits the versioned `plugin-kit-ai/publish-report` contract; `publish --all` stays intentionally absent because local materialization channels and repository/release planning channels still have different execution semantics
