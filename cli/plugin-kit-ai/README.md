...
- `inspect`: explain the discovered package graph, target class, managed artifacts, and the internal publication summary; `--format json` now includes a `publication` block that separates core manifest identity from publication-capable package targets
