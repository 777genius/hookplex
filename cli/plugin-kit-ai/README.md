...
- OpenCode: workspace-config-only lane through `render|import|validate`, package refs, inline MCP, validated portable skills, first-class workspace commands/agents/themes, stable local plugin code plus stable plugin-local package metadata, JSON/JSONC plus explicit user-scope import compatibility, and beta custom tools through plugin code
