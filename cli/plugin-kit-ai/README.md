...
`plugin-kit-ai test` is the stable fixture-driven smoke surface for the declared stable Claude and Codex runtime events. It loads JSON fixtures, invokes the configured launcher entrypoint with the correct carrier, compares `stdout`, `stderr`, and `exitcode` against golden files when they are configured, and supports `--update-golden` to record the current outputs.
