...
- `publish`: first-class bounded publish workflow for local catalog-safe publication channels; `publish --channel codex-marketplace|claude-marketplace --dest <marketplace-root>` materializes the corresponding local marketplace root, `--dry-run` previews the write, and Gemini remains publication-doctor driven because its official model is repository/release based rather than local-catalog based
