...
For the `plugin-kit-ai` CLI itself, the recommended local install path is `brew install 777genius/homebrew-plugin-kit-ai/plugin-kit-ai`. The verified fallback bootstrap path is `scripts/install.sh`, and the official CI setup path is `777genius/plugin-kit-ai/setup-plugin-kit-ai@v1`. Those surfaces install the CLI itself and stay separate from `plugin-kit-ai install`.
