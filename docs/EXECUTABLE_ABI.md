...
For the `plugin-kit-ai` CLI itself, the recommended package-manager install path is Homebrew: `brew install 777genius/homebrew-plugin-kit-ai/plugin-kit-ai`. The verified fallback bootstrap path is `scripts/install.sh`, and the official CI setup path is `777genius/plugin-kit-ai/setup-plugin-kit-ai@v1`. These install the CLI itself; they do not widen `plugin-kit-ai install`, which remains the stable binary-only installer for third-party plugin binaries.
