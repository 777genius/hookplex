...
Use Homebrew to install the `plugin-kit-ai` CLI locally when possible, use `scripts/install.sh` as the verified fallback, and use `777genius/plugin-kit-ai/setup-plugin-kit-ai@v1` to bootstrap the same verified CLI in CI.
