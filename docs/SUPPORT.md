...
- Gemini: packaging-only target through `plugin-kit-ai render|import`; not a production-ready runtime target
...
- repo-root `plugin.yaml` is the canonical authoring manifest for newly scaffolded plugin projects
- the supported `plugin.yaml` v1 schema is intentionally limited to fields with real plugin-kit-ai semantics; unknown or deprecated keys warn in `plugin-kit-ai validate`
- `plugin-kit-ai normalize` is the canonical cleanup path for rewriting deprecated or unknown manifest content into the supported v1 shape
...
- Gemini extension manifest generated by `plugin-kit-ai render --target gemini`, with packaging-only status in the current contract
- `.plugin-kit-ai/project.toml` remains a legacy compatibility manifest for older executable projects and migration/import flows
