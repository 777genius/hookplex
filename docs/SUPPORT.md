...
- OpenCode: workspace-config lane through `plugin-kit-ai render|import|validate`, `opencode.json.plugin`, inline `mcp`, validated mirrored `.opencode/skills/`, and JSON/JSONC native config import; not a production-ready runtime target
...
- OpenCode workspace-config lane through `plugin-kit-ai render|import|validate`, covering official-style `opencode.json` and `opencode.jsonc`, package refs, inline `mcp`, validated portable skills mirrored into `.opencode/skills/`, compatibility import from `.claude/skills` and `.agents/skills`, `config.extra.json`, passthrough config surfaces like `agent` and `tools`, and explicit warnings for unsupported local plugin code plus unsupported workspace directories such as `agents`, `commands`, `themes`, and `modes`
