...
- live Codex CLI evidence currently splits into two buckets:
  - confirmed working paths: real `codex exec` with explicit `-c notify=...` override still reaches the repository-owned notify hook harness, real `codex mcp get --json` with explicit `-c mcp_servers...` overrides still exposes the projected MCP server contract, and a rendered `codex-package` `.mcp.json` sidecar can be projected into those documented overrides for real `mcp get` plus `exec` MCP smoke
