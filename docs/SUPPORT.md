...
- live Codex CLI evidence currently splits into two buckets:
  - confirmed working paths: real `codex exec` with explicit `-c notify=...` override still reaches the repository-owned notify hook harness, the checked-in `examples/plugins/codex-basic-prod` runtime also passes that real `codex exec` path after rebuild, real `codex mcp get --json` and `codex mcp list --json` with explicit `-c mcp_servers...` overrides still expose the projected MCP server contract, and a rendered `codex-package` `.mcp.json` sidecar can be projected into those documented overrides for real `mcp get`, `mcp list`, plus `exec` MCP smoke
  - evidence-only project-config probes: on the current live Codex CLI build (`v0.117.0` in repo evidence), `codex exec` and `codex mcp get` do not reliably honor project-local `.codex/config.toml`; the corresponding opt-in live tests therefore record `skip` with captured output instead of claiming support that the vendor CLI did not prove
