...
| Quality gates | partial | `required`, `extended`, and `live` lanes now exist in repo automation. Remaining gap: prove runner strategy and release usage in practice. |
...
| Security and diagnostics | partial | Threat model, diagnostics contract, checksum verification, and regression coverage exist. Remaining gap: carry them through the stable-candidate audit and close remaining accepted gaps deliberately. |
